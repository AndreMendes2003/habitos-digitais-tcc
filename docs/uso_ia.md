...
| 2026-09-20 | `lib/ui/casca_app.dart` | Gerado por IA. Casca com NavigationBar de tres abas (Casa/Foco/Sequencia) sobre um unico EstadoApp; dona do WidgetsBindingObserver do RNF01, que subiu da TelaFoco. Abre na aba Foco quando ha sessao em andamento. |
| 2026-09-20 | `lib/ui/tela_casa.dart`, `lib/ui/tela_sequencia.dart` | Gerados por IA a partir do conteudo que ja existia na TelaFoco: mascote + linha de uso na Casa, historico de sessoes na Sequencia. |
| 2026-09-20 | `lib/ui/tela_foco.dart` | Reescrita por IA: virou so a aba Foco (seletor, cronometro, botao, resultado), sem Scaffold nem AppBar proprios. |
...
| 2026-09-22 | `lib/estado/estado_mascote.dart` | Modificado por IA: o gatilho de abertura a frio deixou de rodar dentro do construtor e passou a ser agendado para depois do primeiro frame (`addPostFrameCallback`), com o agendador injetavel. Nenhuma regra de penalidade ou de gravacao mudou. |
| 2026-09-22 | `test/estado/estado_app_test.dart`, `test/ui/tela_foco_test.dart`, `test/ui/tela_sequencia_test.dart` | Modificados por IA: passam um agendador sincrono, porque constroem o EstadoApp fora de um frame. Um teste novo cobre o adiamento em si. |
...
| 2026-09-22 | `android/app/src/main/kotlin/com/example/habitos_digitais/MainActivity.kt` | Modificado por IA: MethodChannel `habitos_digitais/tela` que devolve PowerManager.isInteractive e KeyguardManager.isKeyguardLocked. So os valores crus; nenhuma decisao no lado Android. |
| 2026-09-22 | `lib/plataforma/estado_tela.dart` | Gerado por IA. Adaptador do canal, com fallback para tela LIGADA quando o canal nao responde. |
| 2026-09-22 | `lib/dominio/controlador_sessao.dart` | Modificado por IA: `aoMudarCicloDeVida` recebe `telaLigada` (padrao true) e so interrompe no `paused` com a tela ligada; no `resumed` chama verificarProgresso, para concluir sessao que passou do alvo com a tela apagada. Continua sem conhecer MethodChannel. |
| 2026-09-22 | `lib/estado/estado_mascote.dart` | Modificado por IA: consulta o ServicoTela antes de repassar o `paused` e estende o log [LIFECYCLE] com isInteractive/isKeyguardLocked. |
| 2026-09-22 | `test/dominio/controlador_sessao_test.dart` | Testes gerados por IA (4 casos): paused com tela desligada nao interrompe, com tela ligada interrompe, sessao que passa do alvo conclui no resumed, e a volta antes do alvo segue em andamento. |
